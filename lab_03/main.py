...
class Solver:
    ...
    def count_coeffs(self):
        def count_M0():
            ...
            sum2 = self.h * self.p_n_plus_half(0) * self.V_n_plus_half(0) / 8
            sum3 = self.h * self.p_func(self.z_array[0]) / 4
        ...
        def count_P0():
            ...
            sum2 = self.h * self.V_n_plus_half(0) * (self.f_func(self.z_array[0]) + self.f_n_plus_half(0)) / 4
            return -(sum1 + sum2)

        def count_KN():
            sum1 = self.z_n_minus_half(self.N) * self.kappa_n_minus_half(self.N) / (R * R * self.h)
            sum2 = self.h * self.p_n_minus_half(self.N) * self.V_n_minus_half(self.N) / 8
            return sum1 - sum2

        def count_MN():
            sum1 = self.z_n_minus_half(self.N) * self.kappa_n_minus_half(self.N) / (R * R * self.h)
            sum2 = self.z_array[self.N] * self.alpha / R
            sum3 = 3 * self.h * self.p_n_minus_half(self.N) * self.V_n_minus_half(self.N) / 8
            return - (sum1 + sum2 + sum3)

        def count_PN():
            sum1 = 0  # betta = 0
            sum2 = (self.h * self.V_n_minus_half(self.N) *
    ...
    def V_n_minus_half(self, n):
        return self.func_n_minus_half(self.Vn, n)

    def V_n_plus_half(self, n):
        return self.func_n_plus_half(self.Vn, n)
    ...
    def Bn(self, n):
        return self.An(n) + self.Cn(n) + self.p_func(n) * self.Vn(n)

    def Dn(self, n):
        return self.f_func(n) * self.Vn(n)
